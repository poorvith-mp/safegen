...
export function createAgentServer(brokerUrl?: string): McpServer {
  ...
  const server = new McpServer({ name: 'safegen', version: '3.0.0' });
  ...
  server.registerTool('safegen_request_action', {
    description: 'Request one GitHub or Cloudflare action. The owner must approve the exact request in their separate local SafeGen control page. Returns only a pending request ID; never request passwords or control-page links.',
    inputSchema: z.object({
      connection: z.string().max(48), action: z.enum(['github.run-status', 'github.rerun', 'cloudflare.deployments', 'cloudflare.deploy-version']),
      runId: z.number().int().positive().optional(), versionId: z.string().uuid().optional(),
    }).strict(),
