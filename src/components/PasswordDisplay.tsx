...
export const PasswordDisplay: React.FC<PasswordDisplayProps> = ({
  ...
  const renderColoredPassword = (pwd: string) => {
    if (!pwd) {
      const noCharacterSets = options.mode === 'random'
        && !options.uppercase
        && !options.lowercase
        && !options.numbers
        && !options.symbols;
      return <span className="text-slate-400 italic">{noCharacterSets ? 'Choose at least one character set below' : 'No credential available'}</span>;
    }
  ...
  return (
    <div className="w-full max-w-4xl mx-auto bg-white border border-slate-200 rounded-xl p-6 sm:p-8 shadow-xs transition-all">
      ...
      <div className="flex items-center justify-between pb-4 mb-6 border-b border-slate-200 text-xs font-mono uppercase tracking-wider text-slate-500">
        ...
        <div className="flex items-center gap-2">
          <span>{audit.entropy} estimated bits</span>
          <span
      ...
      <div className="relative group my-2">
        <div
          ...
          role="status"
          aria-live="polite"
          aria-label={password ? `Generated ${options.mode} credential` : 'No credential generated'}
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mt-6">
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 w-full sm:w-auto">
          ...
          >
            ...
            <kbd className="ml-1 opacity-80 text-[10px] bg-emerald-700 border-0 px-1.5 py-0.5 rounded text-white">Ctrl/⌘C</kbd>
          </button>
        ...
        <button
          ...
          type="button"
          aria-label="Shuffle generator options"
