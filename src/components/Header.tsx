...
export function Header({ currentView, onNavigate }: HeaderProps) {
  return <header className="instrument-header">
    ...
    <nav aria-label="SafeGen views">{([{ id: 'generator', label: 'Generator' }, { id: 'vault', label: 'History' }, { id: 'audit', label: 'Estimate' }, { id: 'docs', label: 'Docs' }] as const).map((item) => <button key={item.id} aria-current={currentView === item.id ? 'page' : undefined} onClick={() => onNavigate(item.id)}>{item.label}</button>)}</nav>
